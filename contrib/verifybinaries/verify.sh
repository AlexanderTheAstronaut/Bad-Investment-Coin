...
###   badinvestmentcoincore.org and badinvestmentcoin.org and compares them.
...
WORKINGDIR="/tmp/badinvestmentcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://badinvestmentcoincore.org"
HOST2="https://badinvestmentcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="badinvestmentcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'badinvestmentcoin-',
...
if [ $? -ne 0 ]; then
   echo "badinvestmentcoin.org failed to provide signature file, but badinvestmentcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "badinvestmentcoin.org and badinvestmentcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Badinvestmentcoin Core binary release signing key installed?"
   fi
