...
const char * const BADINVESTMENTCOIN_CONF_FILENAME = "badinvestmentcoin.conf";
const char * const BADINVESTMENTCOIN_PID_FILENAME = "badinvestmentcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "badinvestmentcoind -foo=bar
...
#else
    const char* pszModule = "badinvestmentcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Badinvestmentcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Badinvestmentcoin
    // Mac: ~/Library/Application Support/Badinvestmentcoin
    // Unix: ~/.badinvestmentcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Badinvestmentcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Badinvestmentcoin";
#else
    // Unix
    return pathRet / ".badinvestmentcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BADINVESTMENTCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BADINVESTMENTCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Badinvestmentcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Badinvestmentcoin Core") == std::string::npos) {
